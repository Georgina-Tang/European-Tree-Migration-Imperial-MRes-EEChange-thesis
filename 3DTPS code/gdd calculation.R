...
subset1_3 <- subset1_2 %>%
  dplyr::filter(!is.na(se_gdd_anomaly))#486 out of 714 left
subset1_3 <- subset1_3 %>%
  dplyr::filter(!is.na(gdd_anomaly))#486 out of 714 left


#turns out that the weights need to be a vector

vec1 <- subset1_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_1_elv_w <- subset1_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_1_elv_w, file='tps_gdd_1_elv_w.rdata')

summary(tps_gdd_1_elv_w$gdd_anomaly)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-1.48809  0.03775  0.32567  0.43900  0.81579  3.61400 

tps_gdd_1_elv_w_cut<- tps_gdd_1_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset2_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_2_elv_w <- subset2_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_2_elv_w, file='tps_gdd_2_elv_w.rdata')

summary(tps_gdd_2_elv_w$gdd_anomaly)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-5.10114 -0.61900  0.09482 -0.03155  0.76074  1.51026 

tps_gdd_2_elv_w_cut<- tps_gdd_2_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset3_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_3_elv_w <- subset3_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_3_elv_w, file='tps_gdd_3_elv_w.rdata')

summary(tps_gdd_3_elv_w$gdd_anomaly)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
#-5.83178 -0.68166  0.07915  0.21380  1.42776  3.00216 

tps_gdd_3_elv_w_cut<- tps_gdd_3_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset4_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_4_elv_w <- subset4_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_4_elv_w, file='tps_gdd_4_elv_w.rdata')

summary(tps_gdd_4_elv_w$gdd_anomaly)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-4.8426 -0.5928  0.3477  0.3428  1.4086  3.6820 

tps_gdd_4_elv_w_cut<- tps_gdd_4_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset5_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_5_elv_w <- subset5_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_5_elv_w, file='tps_gdd_5_elv_w.rdata')

summary(tps_gdd_5_elv_w$gdd_anomaly)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-3.9726 -0.1719  0.4402  0.5754  1.2878  4.2928 

tps_gdd_5_elv_w_cut<- tps_gdd_5_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
subset6_3 <- subset6_2 %>%
  dplyr::filter(!is.na(se_gdd_anomaly))#272 out of 601 left


vec1 <- subset6_3$se_gdd_anomaly
#vec1 <-1/vec1 
vec2 <-1/(vec1*vec1)
#using se_gdd_anomaly as weighting
tps_gdd_6_elv_w <- subset6_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_6_elv_w, file='tps_gdd_6_elv_w.rdata')

summary(tps_gdd_6_elv_w$gdd_anomaly)
#     Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-4.7289 -1.4124 -0.6841 -0.4164  0.6162  3.7253 


tps_gdd_6_elv_w_cut<- tps_gdd_6_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
#190994 out of 683455 left
...
vec1 <- subset7_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_7_elv_w <- subset7_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_7_elv_w, file='tps_gdd_7_elv_w.rdata')

summary(tps_gdd_7_elv_w$gdd_anomaly)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-8.2973 -2.5615 -1.5055 -1.5807 -0.5073  4.2795 

tps_gdd_7_elv_w_cut<- tps_gdd_7_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset8_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_8_elv_w <- subset8_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_8_elv_w, file='tps_gdd_8_elv_w.rdata')

summary(tps_gdd_8_elv_w$gdd_anomaly)
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-8.976  -4.666  -2.831  -2.954  -1.641   3.781 

tps_gdd_8_elv_w_cut<- tps_gdd_8_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset9_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_9_elv_w <- subset9_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_9_elv_w, file='tps_gdd_9_elv_w.rdata')

summary(tps_gdd_9_elv_w$gdd_anomaly)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-6.020  -4.961  -4.271  -3.383  -2.286   4.263 

tps_gdd_9_elv_w_cut<- tps_gdd_9_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset10_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_10_elv_w <- subset10_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_10_elv_w, file='tps_gdd_10_elv_w.rdata')

summary(tps_gdd_10_elv_w$gdd_anomaly)
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-11.593  -5.856  -4.880  -4.734  -3.942   2.086 

tps_gdd_10_elv_w_cut<- tps_gdd_10_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset11_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_11_elv_w <- subset11_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_11_elv_w, file='tps_gdd_11_elv_w.rdata')

summary(tps_gdd_11_elv_w$gdd_anomaly)
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-15.336  -9.826  -6.789  -5.507  -1.398   9.045 

tps_gdd_11_elv_w_cut<- tps_gdd_11_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
...
vec1 <- subset12_3$se_gdd_anomaly
#vec1 <-1/vec1
vec2 <-1/(vec1*vec1)


#using se_gdd_anomaly as weighting
tps_gdd_12_elv_w <- subset12_3 %>%
  smpds::tps(
    var = "gdd_anomaly",
    z_var = "elv",
    z_mode = "independent",
...
save(tps_gdd_12_elv_w, file='tps_gdd_12_elv_w.rdata')

summary(tps_gdd_12_elv_w$gdd_anomaly)
#   Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#-21.124 -11.658  -8.826  -8.143  -3.859   5.936 

tps_gdd_12_elv_w_cut<- tps_gdd_12_elv_w  %>%dplyr::filter(longitude>-12&longitude<45&latitude<73)
